...
class PdfController extends Controller
{
    ...
    public function payment_slip_pdf(Request $request){
        ...
        if($categoryId == 1){
            ...
            ->leftjoin('users','dambadiwa_crew_payments.crewId','users.id')
            ->leftjoin('user_contact_infos','users.id','user_contact_infos.userId')
            ->leftjoin('dambadiwa_projects','dambadiwa_crew_payments.project_id','dambadiwa_projects.id')
            ->leftjoin('dambadiwa_crews','dambadiwa_crew_payments.project_id','dambadiwa_crews.projectId')
            ->where('dambadiwa_crew_payments.active','=',1)
            ->where('users.active','=',1)
            ->where('dambadiwa_crew_payments.id','=',$payment_id)
            ->where('dambadiwa_crew_payments.project_id','=',$projectId)
            ->where('dambadiwa_crew_payments.crewId','=',$crewId)
            ->where('dambadiwa_crew_payments.categoryId','=',$categoryId)
            ->select('dambadiwa_crew_payments.*','users.*','user_contact_infos.*','dambadiwa_projects.startDate','dambadiwa_crews.id AS regId')
            ->first();
        }
        else if($categoryId == 2){
            $project_payments = DB::table('dambadiwa_crew_payments')
            ->leftjoin('followers','dambadiwa_crew_payments.crewId','followers.id')
            ->leftjoin('contact_infos','followers.id','contact_infos.followerId')
            ->leftjoin('dambadiwa_projects','dambadiwa_crew_payments.project_id','dambadiwa_projects.id')
            ->leftjoin('dambadiwa_crews','dambadiwa_crew_payments.project_id','dambadiwa_crews.projectId')
            ->where('dambadiwa_crew_payments.active','=',1)
